"""Compare Cellpose-SAM and nnU-Net masks at fixed full-image locations."""
...
def main() -> None:
    ...
    if image.shape != cellpose.shape or image.shape != nnunet.shape:
        raise ValueError(
            f"Shape mismatch: image={image.shape}, Cellpose-SAM={cellpose.shape}, nnU-Net={nnunet.shape}"
        )

    figure, axes = plt.subplots(4, 3, figsize=(10.5, 13.0))
    size = args.crop_size
